...
public class LightFlicker : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.SetPositionAndRotation(m_InitialPosition + Noise * m_PositionJitterScale,
            m_InitialRotation * Quaternion.Euler(Noise * m_RotationJitterScale));
    ...
    private Vector3 PerlinNoise3D(Vector3 uv, int Octaves, float freq)
    {
        ...
        {
            float noiseScale = freq * (i + 1);
            output.x += Mathf.PerlinNoise(uv.x * noiseScale, 0f);
            output.y += Mathf.PerlinNoise(uv.y * noiseScale, 0f);
            output.z += Mathf.PerlinNoise(uv.z * noiseScale, 0f);
        }
